...
<li><a href="../phpCode/logout.php">logout</a></li>
       ...
       if (isset($_POST['action-test'])) {
           //collect form data
           $aname = $_POST['customer_name'];
           $aLname = $_POST['customer_second_name'];
           $aContact = $_POST['customer_contact'];
           $alocation = $_POST['customer_location'];
           $cEvent = $_POST['customer_event'];
           ...
           if (in_array($fileActualExtension, $allowed)) {
               if ($fileError == 0) {
                   if ($fileSize < 4000000) {
                       ...
                       $fileDestination = "../images/profileImages/customer/".$fileNameNew;
                       ...
                       header("Location: /customerFormatter.php?error=fileSizeTooLarge");
                       exit();
                   }
               } else {
                   header("Location: /customerFormatter.php?error=fileUploadError");
                   exit();
               }
           } else {
               header("Location: /customerFormatter.php?error=imageTypeNotSupported");
               exit();
           }


           if (empty($aname) || empty($aLname) || empty($aContact)) {
               header("Location: /customerFormatter.php?error=emtpyfields");
               exit();
           } elseif (!preg_match("/^(?=.*[a-z])(?=.*[A-Z])[a-zA-Z]{3,}$/", $aname)) {
               header("Location: ../customerFormatter.php?error=customerNameNotCorrect");
               exit();
           } elseif (!preg_match("/^(?=.*[a-z])(?=.*[A-Z])[a-zA-Z]{3,}$/", $aLname)) {
               header("Location:  ../customerFormatter.php?error=customerLastNotCorrect");
               exit();
           } elseif (!preg_match("/^\d{10}$/", $aContact)) {
               header("Location: ../customerFormatter.php?error=contactIncorrect");
               exit();
           } else {
               $query = "select username from customer where username = ?";
               $stmt = mysqli_stmt_init($con);
               if (!mysqli_stmt_prepare($stmt, $query)) {
                   header("Location: ../customerFormatter.php?error=sqlError");
                   ...
                   if ($resultCheck > 0) {
                       header("Location: ../customerFormatter.php?error=Username Not available");
                       exit();
                   } else {
                       $query1 = "insert into customer (username,customerName,customerLastName,customerContact,customerLocation,customerEvent) values (?,?,?,?,?,?)";

                       $stmtInsert = mysqli_stmt_init($con);
                       if (!mysqli_stmt_prepare($stmtInsert, $query1)) {
                           header("Location: ../customerFormatter.php?error=InsertError");
                           exit();
                       } else {
                           mysqli_stmt_bind_param($stmtInsert, "ssssss", $username1, $aname, $aLname, $aContact, $alocation, $cEvent);
                           mysqli_stmt_execute($stmtInsert);
                           header("Location: ../profilePages/customerProfile.php?LoginStatus=successOfSeeker");
                           exit();
                       }
                   }
               }
